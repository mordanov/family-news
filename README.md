...
Приложение для хранения семейных новостей — с медиа-вложениями (фото, видео, аудио), цветными карточками и удобным интерфейсом.
...
| POST | `/api/news` | Создать новость (multipart/form-data, `media[]`) |
| GET | `/api/news/{id}` | Получить одну новость |
| PUT | `/api/news/{id}` | Обновить новость (`new_media[]`) |
...
Для вложений поддерживаются поля `media` / `new_media` (а также старые `photos` / `new_photos` для совместимости).
Поддерживаемые типы файлов: изображения (`image/*` из списка на backend), видео (`video/mp4`, `video/webm`, `video/quicktime`) и аудио (`audio/mpeg`, `audio/wav`, `audio/ogg`, `audio/mp4`, `audio/x-m4a`).
